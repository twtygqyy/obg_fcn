...
class SBDDSegDataLayer(caffe.Layer):
    ...
    def setup(self, bottom, top):
        ...
        # two tops: data and label
        if len(top) != 3:
            raise Exception("Need to define three tops: data ,label and contour label.")
        # data layers have no bottoms
        if len(bottom) != 0:
            raise Exception("Do not define a bottom.")
    ...
    def reshape(self, bottom, top):
        ...
        self.data = self.load_image(self.indices[self.idx])
        self.label = self.load_label(self.indices[self.idx])
        self.contourlabel = self.load_contourlabel(self.indices[self.idx])
        # reshape tops to fit (leading 1 is for batch dimension)
        top[0].reshape(1, *self.data.shape)
        top[1].reshape(1, *self.label.shape)
        top[2].reshape(1, *self.contourlabel.shape)
