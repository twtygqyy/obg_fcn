...
def fcn(split):
    ...
    n.upscore_pool2 = L.Deconvolution(n.fuse_pool2,
        convolution_param=dict(num_output=21, kernel_size=4, stride=2,
            bias_term=False),
        param=[dict(lr_mult=0)])
    n.score_pool1 = L.Convolution(n.pool1, num_output=21, kernel_size=1, pad=0,
        param=[dict(lr_mult=1, decay_mult=1), dict(lr_mult=2, decay_mult=0)])
    n.score_pool1c = crop(n.score_pool1, n.upscore_pool2)
    n.fuse_pool1 = L.Eltwise(n.upscore_pool2, n.score_pool1c,
            operation=P.Eltwise.SUM)
            
    n.upscore2 = L.Deconvolution(n.fuse_pool1,
        convolution_param=dict(num_output=21, kernel_size=4, stride=2,
            bias_term=False),
        param=[dict(lr_mult=0)])

    n.score = crop(n.upscore2, n.data)
...
def make_net():
    with open('train2.prototxt', 'w') as f:
